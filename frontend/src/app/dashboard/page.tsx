...
export default function DashboardPage() {
    ...
    const { stats, recentActivity, todayInterviews, dueFollowupsList } = data?.data || {};
    ...
    return (
        <div className="space-y-8">
                            ...
                            >
                                <div className="flex-1 min-w-0">
                                    <p className="font-medium text-text-primary truncate">
                                        {interview.application.jobRole || 'Interview'}
                                    </p>
                                    <p className="text-sm text-text-secondary truncate">
                                        {interview.application.hiringCompany || 'Unknown Company'}
                                    </p>
                                    <p className="text-sm text-text-secondary">
                                        Round: {interview.roundName || interview.round_name}
                                    </p>
                                </div>
                                <div className="text-right">
                                    <span className="inline-flex px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800">
                                        {interview.scheduledDate || interview.scheduled_date ? new Date(interview.scheduledDate || interview.scheduled_date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'TBD'}
                                    </span>
                                </div>
                            </Link>
                        ))}
            ...
            {/* Alerts */}
            {(dueFollowupsList && dueFollowupsList.length > 0) && (
                <div className="bg-white rounded-xl shadow-card">
                    <div className="p-6 border-b border-border">
                        <h2 className="text-lg font-semibold text-text-primary flex items-center gap-2">
                            <AlertCircle className="w-5 h-5 text-amber-600" />
                            Pending Follow-ups ({dueFollowupsList.length})
                        </h2>
                    </div>
                    <div className="divide-y divide-border">
                        {dueFollowupsList.map((followup: any) => (
                            <Link
                                key={followup.id}
                            ...
                            >
                                <div className="flex-1 min-w-0">
                                    <p className="font-medium text-text-primary truncate">
                                        {followup.application.jobRole || 'Follow-up'}
                                    </p>
                                    <p className="text-sm text-text-secondary truncate">
                                        {followup.application.hiringCompany || 'Unknown Company'}
                                    </p>
                                    <p className="text-sm text-amber-600">
                                        Due: {followup.followUpDate ? new Date(followup.followUpDate).toLocaleDateString() : 'N/A'}
                                    </p>
                                </div>
